...
def calculate(list):
    ...
    meanCOL = np.mean(arr3,axis=0).tolist() # COL stands for column
    meanROW = np.mean(arr3,axis=1).tolist() # ROW stands for row
    meanFLA = np.mean(arr).tolist()         # FLA stand for flattened
    meanTOL = [meanCOL,meanROW,meanFLA]     # TOL stands for total
